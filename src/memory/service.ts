...
function validate(input: MemoryInput, proposal: MemoryProposal, maximumClaims: number): void {
  ...
  const batchSenders = new Set(input.messages.map(({ senderId }) => senderId));
...
export function createMemoryService(options: MemoryServiceOptions): MemoryService {
  ...
  const apply = async (
    ...
    // one of the proposed native ids.
    ...
    for (const entity of proposal.entities) {
      const existing = entity.nativeIds.map((id) => byNativeId.get(id)).find(Boolean);
    ...
    const operations = proposal.claims.map((proposed) => {
      ...
      return proposed.supersedes
        ? {
            operation: "supersede" as const,
            claimId,
            supersedesClaimId: proposed.supersedes.claimId,
            expectedVersion: proposed.supersedes.version,
            value: proposed.value,
            confidence: proposed.confidence,
            evidenceObservationIds: proposed.evidenceObservationIds,
          }
