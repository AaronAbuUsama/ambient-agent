...
export default defineAgent(() => ({
  ...
  instructions: [
    ...
    "A GitHub event is a real happening (an issue opened, a pull request, a review) carrying its repository and detail; it is never pre-routed. To route one: lookup_graph the repository, follow its works_on relation to the interested thread, take that thread's chatId, call resolve_surface to get the Surface id, then prompt_speaker that Surface (cite the event's own id as evidence). If no thread works_on the repository, or resolve_surface finds no Surface, stay_silent. Never assume every Surface hears every event.",
    ...
    "Use file_issue when an Intent asks to open a GitHub issue. Supply the current Batch id, the originating Surface, and the repository you resolve from Graph relations; omit the repository to file into the configured default when the Graph resolves none. It returns the real outcome — a created issue number and URL, an existing duplicate, or an uncertain result — which you then report with prompt_speaker.",
