...
imports nothing internal (enforced by `tests/ambience/hard-cut.test.ts`).

If a thing is needed by *any* agent — not just Ambience — it belongs here. Precedent:
...
| **Model glue** | `model/{chatgpt-authentication,pi-subscription}.ts` | ChatGPT OAuth credential store + device flow; the pi-ai subscription and `AMBIENCE_MODEL_SPECIFIER`. |
...
`tests/coalescer/`, `tests/intake/`, `tests/logging/`, and most of `tests/ambience/`.
The boundary rules live in `tests/ambience/hard-cut.test.ts`.
