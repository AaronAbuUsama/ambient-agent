# apps/server — the Ambience server

The Flue build root: the deployable that hosts Ambience. `vp build --root apps/server`
...
| File | Convention |
|---|---|
| `src/app.ts` | The app: `createAmbientAgentApp` wires `composeAmbience` + dispatch + stores + the Octokit repository, reading its dependencies from the `globalThis` slot the CLI installed (`export default await createAmbientAgentApp(getManagedRuntimeDependencies())`). |
| `src/db.ts` | Flue database convention: `export default sqlite(flueDatabasePath())`. |
| `src/agents/ambience.ts` | Flue agent-discovery convention: a deliberate 3-line re-export of `@ambient-agent/agents/ambience/agent.ts` (the real definition lives beside its dispatch — decision T8). Not a duplicate; do not "fix". |
...
`tests/ambience/{db,whatsapp-runtime}.test.ts`, `tests/managed/smoke-route.test.ts`, and
the fixture app `tests/fixtures/ambience/` mirroring this layout.
