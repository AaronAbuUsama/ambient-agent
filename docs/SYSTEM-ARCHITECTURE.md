...
| **Scribe**                          | One global clock; bounded concurrent stateless attempts; per-Attestation Evidence Sets | Global batch construction and one process-wide four-attempt execution gate serve live observations and Historical Replay. Each attempt now receives trusted stable batch evidence and can append retry-idempotent Entity/Relation proposals without merge authority. Replay has global chronological phase barriers and durable per-Surface scan cutoffs. Live admission still enters an in-memory coalescer while Replay enters through its workflow, so the single durable admission/retry clock is **not built yet**; attempts also lack a fresh bounded input Projection and do not return durable proposal deltas to the Brain                                                                                                     | **Unify live/Replay admission durably; inject a fresh bounded Belief Projection; return durable proposal deltas to the Brain** |
| **Speaker**                         | Dumb mouth: converses only, escalates intent                                           | Intent escalation and Directive-only Saying are built (`intent-escalation/*`, `directive-delivery/*`), and Speaker Graph access is now read-only. The Speaker remains **overloaded** because it still mounts issue-management + delegation (`speaker/agent.ts`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | **Move issue management and delegation to the Brain, then remove those capabilities from the Speaker**                         |
...
| **Coalescer**                       | Modelless timing for Speakers and Scribe                                               | Per-chat Speaker Windows are built in `engine/src/coalescer/*`. Live Scribe observations share one process-wide in-memory coalescer and Replay uses the same batch shape and attempt gate, but the two admission paths are not yet one durable clock                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | **Move both Scribe admission paths onto one durable global queue**                                                             |

**Reading the distance.** The load-bearing implementation primitives already exist: the
append-only Graph and typed Belief Projection, live Digest pull, reactive Brain conversation loop, asynchronous
delegation with durable return, and modelless coalescing. Two corrections remain. First,
finish concentrating routing, work, and ontology authority in the Brain. Second, unify the
Scribe's live and Historical Replay admissions durably and ship fresh bounded Projection context
plus proposal deltas through that clock. The Graph persistence correction is now built; the
remaining distance is concentration of authority and honest delivery boundaries, not a parallel
