...
| **Brain**                           | Single global mind: up-inbox, two clocks, owns state + work                            | The reactive global actor claims immutable mixed Intents, Knowledge Deltas, and Specialist Results in one Batch frontier; owns Graph rulings; records prompt/silence; and owns Coder launch through a stable `brain-work:*` identity. Pending admission reconciles against Flue's snapshotted work id before retry; terminal and interrupted outcomes return durably to the Brain, whose later `prompt_speaker` independently chooses a Surface. GitHub webhook routing and proactive time are not yet owned here                                                                                                                                                                                                                           | **Move GitHub ingress routing onto the Brain; add the proactive clock**                                                        |
...
append-only Graph and typed Belief Projection, bounded live Digest, definitive global Scribe
clock, reactive Brain conversation/knowledge/work loop, asynchronous Brain-owned delegation with durable return,
and modelless coalescing. The remaining correction is to finish concentrating routing, work, and
proactive time in the Brain, then remove the superseded Speaker and routing ownership paths. The
