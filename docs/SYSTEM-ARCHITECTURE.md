...
| **Scribe**                          | One global clock; bounded concurrent stateless attempts; per-Attestation Evidence Sets | Global batch construction and one process-wide four-attempt execution gate now serve live observations and Historical Replay. Replay has global chronological phase barriers and durable per-Surface scan cutoffs; live admission still enters an in-memory coalescer while Replay enters through its workflow, so the single durable admission/retry clock is **not built yet**. Every attempt carries stable evidence membership plus a fresh attempt id, but writes still mutate the current Graph and do not enforce batch idempotency | **Unify live/Replay admission durably; append evidence-set-idempotent Attestations; inject a fresh bounded Belief Projection; return deltas to the Brain** |
