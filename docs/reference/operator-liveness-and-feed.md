...
`WHATSAPP_LIVENESS_BOUND_MS = 65_000` in `packages/installation/src/observation.ts`, on the wire as
`liveness.boundMs`, and asserted in `tests/speaker/whatsapp-runtime.test.ts`.
...
interface WhatsAppLiveness {
  ...
  readonly terminal?: true;   // re-pairing cannot help, or the credential store was wiped
...
| `failed` | terminal (`terminal: true`) or the runtime fiber failed | re-pair, or read `reason` |
...
| `backing_off` | `degraded` | `reason`, `retryAt` carried |
...
| `bridgeHealth` / `bridgePairing` (`bridge-contract.ts`) | the status handed to them |
...
event: gap
data: {"dropped":12,"resumeAfter":43}
```

- `?after=<seq>` resumes a reconnecting client from where it left off. `gap: true` means the
  cursor is older than the ring still reaches — the client missed more than the feed can give it,
  and should go to the log files under `logs/` for the rest.
- **A slow client is dropped, never buffered.** `response.write` returns false once the socket's
  ...
  record is skipped and counted, and the count arrives as a `gap` once the socket recovers. The
  client re-reads what it missed with `?after=`.
...
| `agent.offline` | the reported phase became `failed` or `stopped` | `○` |
...
Only *changes* in the reported phase are narrated: whatsappd emits a transition per retry attempt,
and one line per backoff tick would bury the transition that matters.
