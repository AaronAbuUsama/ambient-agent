...
describe("managed data paths", () => {
  ...
  it("uses the selected platform path dialect for every derived Windows path", () => {
    expect(managedPaths({ platform: "win32", dataDirectory: "D:\\Agent" })).toEqual({
      ...
      managedConfigDatabase: "D:\\Agent\\managed-config.sqlite",
      chatGptOAuthCredential: "D:\\Agent\\credentials\\chatgpt-oauth.json",
      legacyPiAuthCredential: "D:\\Agent\\credentials\\pi-auth.json",
      modelApiKeyCredential: "D:\\Agent\\credentials\\model-api-key.json",
      e2bCredential: "D:\\Agent\\credentials\\e2b.json",
      braintrustCredential: "D:\\Agent\\credentials\\braintrust.json",
      controlPlaneCredential: "D:\\Agent\\credentials\\control-plane.json",
  ...
  it("derives the complete stable skeleton from an injected root", () => {
    ...
    expect(paths).toEqual({
      ...
      managedConfigDatabase: posix.join("/managed", "managed-config.sqlite"),
      ...
      modelApiKeyCredential: posix.join("/managed", "credentials", "model-api-key.json"),
      e2bCredential: posix.join("/managed", "credentials", "e2b.json"),
      braintrustCredential: posix.join("/managed", "credentials", "braintrust.json"),
      controlPlaneCredential: posix.join("/managed", "credentials", "control-plane.json"),
